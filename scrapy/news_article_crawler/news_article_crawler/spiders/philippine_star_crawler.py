...
class CrawlingSpider(CrawlSpider):
    name = "philippine_star_crawler"
    allowed_domains = ["www.philstar.com"]  # Add the allowed domain(s)
    ...
    def parse_item(self, response):
        ...
        if dash_count > 3 and current_url not in self.crawled_links:
            ...
            with open('philstar_links.txt', 'a') as f:
                f.write(current_url + "\n")
    ...
    def is_blocked_domain(self, url):
        ...
        for domain in blocked_domains:
            if domain in url or "philstar.com" not in url:
